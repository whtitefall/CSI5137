...
import sys
sys.path.append('../')
from ufunc.ufunc import loadWord2Vec, clean_str
...
datasets = ['20ng', 'R8', 'R52', 'ohsumed', 'mr','p1','p2']
# build corpus
dataset = sys.argv[1]
...
for i in range(len(shuffle_doc_words_list)):
    sentence_embeddings = model.encode(shuffle_doc_words_list)
    for j in range(len(shuffle_doc_words_list)):

        if i==j and i < train_size:
            weight.append(1)
        elif i < train_size:
      
            c_sim = cosine_similarity([sentence_embeddings[i]],[sentence_embeddings[j]])
            # print(type(c_sim),c_sim)
            weight.append(c_sim[0][0])
        if i < train_size:
            row.append(i)
